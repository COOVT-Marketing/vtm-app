...
            normalized = str(col).strip().lower().replace("*", " ").replace("-", " ")
...
        bad_df = df[~mask_good].drop(columns=["*normalized_phone"])
        # 1. Create new tab with full file
        tab_title = sanitize_sheet_title(filename)
        create_log_tab(service, sheet_id, tab_title, df.drop(columns=["*normalized_phone"]))
...
        good_path = os.path.join("temp_downloads", f"good*{unique_id}.csv")
        bad_path = os.path.join("temp_downloads", f"bad*{unique_id}.csv")
...
        return "Invalid", 400
    file_path = session.get(f"{kind}*file")
    if not file_path or not os.path.exists(file_path):
        flash("No data available. Please process a file first.", "error")
        return redirect(url_for("dashboard"))
    filename = f"{kind}*data*{datetime.now().strftime('%Y%m%d*%H%M%S')}.csv"
    return send_file(
        file_path,
        mimetype="text/csv",
        as_attachment=True,
